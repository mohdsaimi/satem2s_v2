...
<form method="POST" action="{{ route('admin.updatehmrosaktin', [$hmrosak,'term'=>request('term') ?? null]) }}" {{-- enctype="multipart/form-data" --}}>
            ...
            <div class="row">
                <div class="col-sm-5">
                    <h4 class="card-title mb-0">
                        Maklumat Tindakan Aduan Kerosakan (HM)
                    </h4>
                </div><!--col-->
            </div><!--row-->
            <!--row-->
                ...
                <div class="col">
                    ...
                    <div class="form-group row">
                        <label class="col-md-2 col-form-label"><b>Ulasan/Kerja yang dilakukan/Cadangan</b></label>
                            <div class="col-md">
                                <textarea class="form-control" id="nota_tindakan" name="nota_tindakan" required>{{ $hmrosak->nota_tindakan ?? null}}</textarea>
                            </div><!--col-->
                    </div>
                        ...
                        <label class="col-md-2 col-form-label"><b>Status</b></label>
                            <div class="col-md-3">
                                <select class="form-control" name="status" style="text-transform:uppercase" required>
                                    @if (empty($hmrosak->status))
                                    <option value="">Sila Pilih</option>
                                    @else
                                    <option value="{{ $hmrosak->status}}" style="text-transform:uppercase">{{ $hmrosak->statusrosak->nama_status }}</option>
                                    @endif
